...
import { Bell, Building2, CheckCircle2, KeyRound, ShieldCheck, UserRound, UsersRound } from "lucide-react";
import { fetchAccountOverview, updateAccountProfile, uploadAvatarFile, type AccountOverview } from "../../../api/accountApi";
import type { User } from "../../../shared/types";

type SectionKey = "profile" | "security" | "business" | "permissions" | "notifications";
...
const sections: Array<{ key: SectionKey; label: string; icon: typeof UserRound }> = [
  ...
  { key: "business", label: "Khách sạn", icon: Building2 },
...
export function AccountSettingsPage({ onUserUpdated }: { onUserUpdated?: (patch: Partial<User>) => void }) {
  ...
  const initials = useMemo(() => {
    const name = data?.profile.fullName || "Partner";
  ...
  if (!data) {
    return <div className="py-16 text-center text-sm text-slate-500">Đang tải tài khoản...</div>;
  }
    ...
    <div className="space-y-5">
      <div className="flex flex-col gap-4 border-b border-slate-200 pb-5 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <h1 className="mt-1 text-2xl font-black tracking-tight text-slate-950">Tài khoản đối tác</h1>
        </div>
        <div className="flex items-center gap-3 rounded-md border border-slate-200 bg-white px-3 py-2">
          ...
          <div className="min-w-0">
            <p className="truncate text-sm font-bold text-slate-950">{data.profile.fullName}</p>
      ...
      <div className="grid gap-5 lg:grid-cols-[240px_minmax(0,1fr)]">
            ...
            return (
              <button
                ...
                className={[
                  ...
                  isActive ? "border-indigo-100 bg-indigo-50 text-indigo-700" : "border-transparent text-slate-600 hover:border-slate-200 hover:bg-white hover:text-slate-950",
                ].join(" ")}
              >
                <Icon size={18} />
                {item.label}
        ...
        <section className="min-w-0">
          {active === "profile" && (
            <Panel title="Thông tin cá nhân" description="Thông tin liên hệ chính dùng trong vận hành và khôi phục tài khoản.">
              <div className="grid gap-4 sm:grid-cols-[120px_minmax(0,1fr)]">
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  ...
                  <Field label="Chức vụ" value="Quản lý đối tác" disabled />
              ...
              <div className="mt-5 flex flex-col gap-3 sm:flex-row sm:items-center">
                <button disabled={saving} onClick={saveProfile} className="h-10 rounded-md bg-indigo-600 px-4 text-sm font-bold text-white disabled:opacity-60">
          ...
          {active === "security" && (
            <Panel title="Bảo mật tài khoản" description="Theo dõi phương thức đăng nhập và các phiên đang hoạt động.">
              <div className="grid gap-3 md:grid-cols-3">
                ...
                <StatusTile title="Mật khẩu" value="Có thể đổi qua khôi phục" active />
                <StatusTile title="2FA" value="Chưa bật" active={false} />
              </div>
              <div className="mt-5 overflow-hidden rounded-md border border-slate-200">
                {data.security.sessions.map((session) => (
                  <div key={session.id} className="grid gap-2 border-b border-slate-200 p-4 last:border-b-0 md:grid-cols-[1fr_160px_160px]">
                    <div>
                      <p className="text-sm font-bold text-slate-950">{session.deviceName}</p>
                      <p className="mt-1 truncate text-xs text-slate-500">{session.userAgent || "Không có user agent"}</p>
                    </div>
          ...
          {active === "business" && (
            <Panel title="Thông tin khách sạn / doanh nghiệp" description="Dữ liệu dùng cho hồ sơ đối tác, đối soát và vận hành đặt phòng.">
              {data.business ? (
                <div className="grid gap-4 sm:grid-cols-2">
                  <InfoCard label="Tên khách sạn" value={data.business.hotelName} />
                  <InfoCard label="Trạng thái KYC" value={data.business.kycStatus} />
                  <InfoCard label="Địa chỉ" value={[data.business.address, data.business.city].filter(Boolean).join(", ") || "-"} />
                  <InfoCard label="Hotline" value={data.business.hotline || "-"} />
                  <InfoCard label="Mã số thuế" value={data.business.taxCode || "Chưa cập nhật"} />
                  <InfoCard label="Tài khoản ngân hàng" value={data.business.bankAccountNumber ? `${data.business.bankName || ""} - ${data.business.bankAccountNumber}` : "Chưa cập nhật"} />
                  <InfoCard label="Tên chủ tài khoản" value={data.business.bankAccountName || "Chưa cập nhật"} />
                  <InfoCard label="Gói hoa hồng" value={data.business.commissionTier} />
                </div>
              ) : (
                <div className="rounded-md border border-amber-200 bg-amber-50 p-4 text-sm font-semibold text-amber-800">Tài khoản chưa có hồ sơ đối tác.</div>
              )}
            </Panel>
          )}

          {active === "permissions" && (
            <Panel title="Quản lý phân quyền" description="Quyền hiện tại của tài khoản trong cổng đối tác.">
              <div className="space-y-3">
          ...
          {active === "notifications" && (
            <Panel title="Cấu hình thông báo" description="Chọn cách nhận các thông tin quan trọng về đặt phòng và doanh thu.">
              <div className="space-y-3">
                {data.notifications.map((item) => (
                  <label key={item.key} className="flex items-center justify-between gap-4 rounded-md border border-slate-200 p-4">
...
function Field({ label, value, onChange, disabled }: { label: string; value: string; onChange?: (value: string) => void; disabled?: boolean }) {
  return (
    <label className="space-y-1.5">
      ...
      <input value={value} disabled={disabled} onChange={(event) => onChange?.(event.target.value)} className="h-10 w-full rounded-md border border-slate-200 bg-white px-3 text-sm outline-none focus:border-indigo-500 disabled:bg-slate-50" />
    </label>
  );
}
...
function SelectField({ label, value, onChange }: { label: string; value: string; onChange: (value: string) => void }) {
  return (
    <label className="space-y-1.5">
      <span className="text-xs font-bold uppercase text-slate-500">{label}</span>
      <select value={value} onChange={(event) => onChange(event.target.value)} className="h-10 w-full rounded-md border border-slate-200 bg-white px-3 text-sm outline-none focus:border-indigo-500">
