...
function normalizeRecord(record, seenSlugs) {
  ...
  return {
    ...
    name,
    slug,
    propertyType: record.star_rating ? String(record.star_rating) : "4",
...
function buildSql(records) {
  ...
  const chunks = [
    ...
    "-- Review this file before applying to the database.",
    "START TRANSACTION;",
  ];
  ...
  readyRecords.forEach((record) => {
    ...
    chunks.push(`SET @partner_id := ${DEFAULTS.partnerId};`);
    chunks.push(`SET @property_slug := ${sqlString(record.slug)};`);
    chunks.push(`SET @existing_property_id := (SELECT id FROM properties WHERE slug = @property_slug LIMIT 1);`);
    chunks.push("SET @property_id := NULL;");
    chunks.push(
      [
        "INSERT INTO properties",
        "(partner_id, slug, name, property_type, description, address, city, district, country_code, latitude, longitude, star_rating, status, amenities_json, highlights_json)",
        "SELECT",
        [
          "@partner_id",
          "@property_slug",
          sqlString(record.name),
          sqlString(record.propertyType),
          ...
          sqlJson(record.amenities),
          sqlJson([...record.highlights, ...sourceHighlights(record)]),
        ].join(", "),
        "WHERE @existing_property_id IS NULL;",
      ].join(" ")
    );
    chunks.push("SET @property_id := COALESCE(@existing_property_id, LAST_INSERT_ID());");
    chunks.push(
      "UPDATE properties SET " +
        [
          `avg_rating = ${sqlString(record.avgRating)}`,
          `total_reviews = ${record.totalReviews}`,
          `area_sqm = ${record.areaSqm ? sqlString(record.areaSqm) : "NULL"}`,
          `capacity = ${record.capacity}`,
          `check_in_time = ${sqlString(record.checkInTime)}`,
          `check_out_time = ${sqlString(record.checkOutTime)}`,
          `transport_connections_json = ${sqlJson(record.transportConnections)}`,
        ].join(", ") +
        " WHERE id = @property_id;"
    );
    chunks.push(
      "INSERT IGNORE INTO property_policies (property_id, cancellation_type, free_cancel_hours, check_in_from, check_in_until, check_out_until, pets_allowed, smoking_allowed, children_allowed, children_free_age, cancellation_policy_text) " +
        `VALUES (@property_id, ${sqlString(record.policy.cancellationType)}, ${record.policy.freeCancelHours}, ${sqlString(record.checkInTime)}, '23:59:00', ${sqlString(record.checkOutTime)}, ${record.policy.petsAllowed}, ${record.policy.smokingAllowed}, ${record.policy.childrenAllowed}, ${record.policy.childrenFreeAge}, ${sqlString(record.policy.cancellationPolicyText)});`
    );
    record.rooms.forEach((room, index) => {
      const roomImages = room.images.length > 0 ? room.images : record.images;
      chunks.push(
        "INSERT INTO property_pricing (property_id, label, price_per_night, sort_order, capacity, image_urls_json, image_urls, total_inventory) " +
          `SELECT @property_id, ${sqlString(room.label)}, ${sqlString(room.pricePerNight)}, ${index}, ${room.capacity}, ${sqlJson(roomImages)}, ${sqlJson(roomImages)}, ${room.totalInventory} ` +
          `WHERE NOT EXISTS (SELECT 1 FROM property_pricing WHERE property_id = @property_id AND label = ${sqlString(room.label)});`
      );
      chunks.push(
        "UPDATE property_pricing SET " +
          [
            `area_sqm = ${room.areaSqm ? sqlString(room.areaSqm) : "NULL"}`,
            `area = ${room.areaSqm ? sqlString(room.areaSqm) : "NULL"}`,
            `bed_info = ${sqlString(room.bedInfo)}`,
            `amenities = ${sqlString(record.amenities.join(", "))}`,
          ].join(", ") +
          ` WHERE property_id = @property_id AND label = ${sqlString(room.label)};`
      );
    });
    record.galleryImages.slice(0, 12).forEach((image, index) => {
      chunks.push(
        "INSERT INTO property_gallery_images (property_id, category, image_url, caption, sort_order) " +
          `SELECT @property_id, ${sqlString(image.category)}, ${sqlString(image.url)}, ${sqlString(record.name)}, ${index} ` +
          `WHERE NOT EXISTS (SELECT 1 FROM property_gallery_images WHERE property_id = @property_id AND image_url = ${sqlString(image.url)});`
      );
    });
    record.nearbyPlaces.slice(0, NEARBY_TOTAL_LIMIT).forEach((place, index) => {
      chunks.push(
        "INSERT INTO property_nearby_places (property_id, name, category, distance_m, latitude, longitude) " +
          `SELECT @property_id, ${sqlString(place.name)}, ${sqlString(place.category)}, ${distanceToMeters(place.distance, index)}, ${sqlString(place.latitude || record.latitude)}, ${sqlString(place.longitude || record.longitude)} ` +
          `WHERE NOT EXISTS (SELECT 1 FROM property_nearby_places WHERE property_id = @property_id AND name = ${sqlString(place.name)});`
      );
    });
  });
